...
;
; IO  in:  mlen{1}  mesh{mlen}  ss_dh{32}  ss_kem{32}
; The later steps will read the two plaintexts each length
; prefixed the same way;
; out: h_init{32}  h_after_mesh{32}  ck_after_dh{32}  ck_after_kem{32}
;
...
;
; WHAT IS IMPLEMENTED SO FAR: init mixHash and both mixKey steps __ four of
; the ten frozen values; The remaining six are two AEAD seals two hashes of
; a ciphertext and the split; They land one at a time each
...
; 287   289    the message key after mixKey(ss_kem)
;
