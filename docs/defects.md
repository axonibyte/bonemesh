...
| D4 | A NAK marks the message's final destination dead instead of the next hop that actually failed. | `java/.../BoneMesh.java` (`setNodeStatus`) | protocol (v3) | specified: v3 defines per-hop failure attribution (protocol.md §7). The `ack`/NAK message types are reserved but **not emitted by the 3.0.0 reference nodes** (they signal undeliverability via a failed `send`), so the corrected attribution is not yet exercised — deferred with the ack/NAK machinery. |
