...
## Post-quantum interop — RESOLVED (Java, Elixir, Rust, Go, JS)
...
(`check-pqc-go.sh`), and the JS side (Node/OpenSSL 3.5, `check-pqc-js.sh`):

- **ML-DSA-65 signatures** — Elixir, Rust, Go, and JS each verify a signature
  Java made. Go uses Cloudflare CIRCL (`sign/mldsa/mldsa65`); JS uses Node's
  built-in `crypto` (OpenSSL 3.5) — in each case the same code the node uses on
  the handshake path.
- **ML-KEM-768 cross-decapsulation** — Elixir and Rust decapsulate a ciphertext
  ...
  and RustCrypto). Go and JS prove ML-KEM-768 interop **live** instead: every
  pairing in `interop/run-matrix.sh` completes a real hybrid handshake in which
  they decapsulate ciphertexts made by the other implementations and vice versa.
  Both are keyed by the 64-byte *seed* (Go's stdlib `crypto/mlkem`, Node's
  OpenSSL ML-KEM), so neither ingests the vector's expanded decapsulation key —
  but a decapsulation key never crosses a node, so this is a key-*representation*
  detail, not an interop gap (see below).

So the post-quantum primitives interoperate across five independent
implementations, and the BMX handshake works across all five.
...
RustCrypto, Go, and Node keep the seed. This never affects interop because
private keys never cross a node boundary; only public keys, ciphertexts, and
signatures do, and those are the standard FIPS encodings, which all five match.

## Live cross-language matrix

`interop/run-matrix.sh` runs every (responder, initiator) pair across all five
implementations — Java, Elixir, Rust, Go, JS — completing a real BMX handshake,
encrypted transport, and application delivery in each direction. All 25 cells
...
- **Byte-exact full transcript** (the whole bmx1/bmx2/bmx3 bytes) — a static
  ...
  to end across all five languages. A pinned transcript vector remains optional
  future work for offline conformance.

Nothing frozen is weakened: the key agreement and schedule are proven across
Java, Go, Elixir, Rust, and JS, and the post-quantum layer is now proven across
all five.
