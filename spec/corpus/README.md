...
| `transcripts/` | Full BMX handshake transcripts with fixed keys — freezes the provisional key-schedule constants. | **pending M4** (needs the first two interoperating implementations; see `security.md` top note). |
...
The `transcripts/` vectors are deliberately absent: a handshake transcript is
only meaningful once two implementations can produce and agree on one, so those
constants are frozen at first interop (M4), not guessed now. The active files
cover the entire deterministic wire contract, which is testable today.
